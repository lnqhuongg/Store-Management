...
export default function KHLayout({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        const handler = (e: any) => {
            GlobalStore.setCustomer(e.detail);
        };
        window.addEventListener("select-customer", handler);
        return () => window.removeEventListener("select-customer", handler);
    }, []);
